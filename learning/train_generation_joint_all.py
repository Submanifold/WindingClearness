...
def normal_loss_function(normalized_mu, pca_normals):
    diff1 = normalized_mu + pca_normals  # 求和
    diff2 = normalized_mu - pca_normals  # 差值

    norm_diff1 = torch.norm(diff1, dim=2)  # 计算模长
    norm_diff2 = torch.norm(diff2, dim=2)

    norm_diff1 = torch.unsqueeze(norm_diff1, dim=2)  # 增加一个维度
    norm_diff2 = torch.unsqueeze(norm_diff2, dim=2)

    min_norm = torch.min(norm_diff1, norm_diff2)  # 逐元素比较最小值

    loss = torch.mean(min_norm)  # 计算平均损失

    return loss

def unoriented_normals(point_cloud, device, k=7):
    indices = knn_points(point_cloud, point_cloud, K=k + 1)
    indices = indices.idx  # 获取idx值
    indices = indices.to(device)
    indices_int = indices.int()  # 转换为整数类型
    neighbors = knn_gather(point_cloud, indices_int[:, :, 1:].long())
    centered_neighbors = neighbors - neighbors.mean(dim=2, keepdim=True)
    covariance = torch.matmul(centered_neighbors.transpose(2, 3), centered_neighbors)

    _, eigenvectors = torch.symeig(covariance, eigenvectors=True)
    normals = eigenvectors[:, :, :, 0]  # 最小的特征值对应的特征向量作为法向量

    return normals

def compute_loss(x, device, bound_point_torch, opt):

    bound_point_torch_size = bound_point_torch.size()
    x_size = x.size()
    batch_size = x_size[0]
    num_point = x_size[1]
    b = torch.ones((batch_size, 1, num_point)) * 0.5
    ...
    lhs = AtA + lambda_j * A2tA2 + lambda_regu * (diagonalAtA_matrix.unsqueeze(0) + lambda_j * diagonalA2tA2_matrix.unsqueeze(0))
    rhs = At @ bT
    ...
    pgr_loss = pgr_loss + lambda_regu * torch.bmm(mu.transpose(1, 2),
                                                  torch.bmm(diagonalAtA_matrix.unsqueeze(0), mu)) / num_point
...
def train(gpu, opt, output_dir, noises_init):
    ...
    #will be reset to epoch + 1 + seed in L810
    set_seed(opt)
    ...
    total_point_num = opt.npoints
    bound_point_coord = 0.6
    bound_point_numpy = sample_bound_points(-bound_point_coord, bound_point_coord, -bound_point_coord, bound_point_coord, -bound_point_coord, bound_point_coord, 2 * total_point_num)
    ...
    for epoch in range(start_epoch, opt.niter):
        ...
        #Ensure different seed for joint train and generation
        torch.manual_seed(int(epoch + 1 + opt.manualSeed))
        np.random.seed(int(epoch + 1 + opt.manualSeed))
        for i, data in enumerate(dataloader):
            ...
            loss_diffusion = model.get_loss_iter(x, noises_batch).mean()
            #winding clearness
            x_winding_batch = x[:batch_size_winding_clear, :, :]
            gen = model.gen_samples(x_winding_batch.shape, 'cuda', clip_denoised=False)
            gen = gen.transpose(1, 2).contiguous()
            for j in range(batch_size_winding_clear):
                pc = gen[j, :, :]
                ...
                pc = (pc - shift) / scale
                gen[j, :, :] = pc
            loss_winding_clearness = compute_loss(gen, device, bound_point_torch, opt)
            #total
            loss_winding_clearness_total = sum(loss_winding_clearness) * opt.lambda_joint
            loss_winding_clearness_total = torch.squeeze(loss_winding_clearness_total)
            loss_total = loss_diffusion + loss_winding_clearness_total
            #print("lr: " + str(optimizer.param_groups[0]['lr']))
            print("loss diffusion: " + str(loss_diffusion))
            print("loss_winding_clearness: " + str(loss_winding_clearness_total))
            optimizer.zero_grad()
            loss_total.backward()
            ...
            if i % opt.print_freq == 0 and should_diag:
                ...
                logger.info('[{:>3d}/{:>3d}][{:>3d}/{:>3d}]    loss: {:>10.4f},    '
                             ...
                             .format(
                        epoch, opt.niter, i, len(dataloader),loss_total.item(),
                    netpNorm, netgradNorm,
                        ))

            if (i + 1) % 200 == 0:
                torch.save(model.state_dict(), os.path.join(opt.joint_train_save_path, 'model_epoch_%d_iter_%d_joint.pth' % (epoch, int(i))))
        '''
        ...
        if epoch % 1 == 0 or epoch == opt.niter - 1:
            torch.save(model.state_dict(), os.path.join(opt.joint_train_save_path, 'model_%d_joint.pth' % (epoch)))
...
def parse_args():
    ...
    parser.add_argument('--joint_train_save_path', default='new_joint_train_model_all')
    ...
    parser.add_argument('--lambda_joint', type=float, default=1.0, help='lambda_joint')

    parser.add_argument('--lr', type=float, default=2e-7, help='learning rate for E, default=0.0002')
    parser.add_argument('--beta1', type=float, default=0.5, help='beta1 for adam. default=0.5')
    parser.add_argument('--decay', type=float, default=0, help='weight decay for EBM')
    parser.add_argument('--grad_clip', type=float, default=None, help='weight decay for EBM')
    parser.add_argument('--lr_gamma', type=float, default=0.995, help='lr decay for EBM')

    parser.add_argument('--model', default='./ckpt/generation/all_55_399_2.pth', help="path to model (to continue training)")
